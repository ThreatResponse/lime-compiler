...
module LimeCompiler
  class Repo
    ...
    def initialize(opts)
      ...
      @module_dir = 'modules'
      GPGME::Engine.home_dir = opts[:gpg_home] if opts[:gpg_home]
    ...
    def generate_metadata(mod_path, sig_path)
      ...
      signature_path = if sig_path.nil?
                         ''
                       else
                         "#{@module_dir}/#{sig_path.split('/')[-1]}"
                       end

      metadata[:name]      = module_name
      metadata[:arch]      = 'x86_64' # TODO: make this dynamic
      metadata[:checksum]  = sha256 mod_path
      metadata[:version]   = mod_name module_name
    ...
    def generate_repodata(base)
      ...
      primary_path = write_primary_metadata base
      primary_checksum = sha256 primary_path

      result = rename primary_path, primary_checksum

      gzip_path = "#{result[:path]}.gz"
      gzip_filename = "#{result[:filename]}.gz"

      timestamp = compress result[:path], result[:filename], gzip_path

      repodata = Nokogiri::XML::Builder.new(encoding: 'UTF-8') do |xml|
        xml.metadata do
          xml.revision Time.now.to_i
          xml.data(type: 'primary') do
            xml.checksum sha256 gzip_path
    ...
    def write_primary_metadata(base)
      primary = Nokogiri::XML::Builder.new(encoding: 'UTF-8') do |xml|
        xml.modules do
          @primary_metadata.each do |_, v|
            xml.module(type: 'lime') do
              xml.name v[:name]
              xml.arch v[:arch]
    ...
    def check_repomd(base)
      ...
      if checksum_matches gzfile, gz_checksum
        primary = nil
        xml_string = nil
        ...
        if checksum_matches xml_string, open_checksum, is_file: false
          @logger.debug "verification complete, merging #{gzfile}"
          merge_repos base, primary['modules']['module']
        else
      ...
      return unless File.file? gzfile
    ...
    def merge_repos(base, modules)
      modules.each do |mod|
        ...
        next unless !@primary_metadata.key?(mod_path) && File.file?(mod_path)
        ...
        if mod['signature']['href'] != ''
          ...
          sig_pass = if verify_signature mod_path, sig_path
                       true
                     else
                       false
                     end
        ...
        checksum_pass = checksum_matches mod_path, mod['checksum']

        # only insert module if verification passes
        if sig_pass && checksum_pass
          @logger.debug "injecting module metadata for #{mod_path}"
          generate_metadata mod_path, sig_path
        else
          sig = sig_pass && sig_attempt_verify
    ...
    def checksum_matches(data, checksum, opts = { is_file: true })
      calculated = if opts[:is_file]
                     sha256 data
                   else
                     Digest::SHA256.hexdigest data
                   end
      @logger.debug "calculated checksum #{calculated}"
      checksum.eql?(calculated)
    end
  end
end
