...
module LimeCompiler
  class CompileTarget
    ...
    def pre_actions
      return if @distro[:pre_actions].nil?
    ...
    def compile_lime
      ...
      @packages.each do |package|
        if @distro[:source_include_arch?]
          ...
          kernel = package[@distro[:source_strip].length, package.length].to_s
        end

        if !module_built?(kernel) || @build_all
